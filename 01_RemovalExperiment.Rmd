...
# Reproducible setup
This analysis was run with: glmmTMB 1.1.8, TMB 1.9.10, Matrix 1.5-4.
...
# Create simplified formulas by dropping non-significant terms sequentially (stepwise, one at the time)
# STEP 1 - drop first term (3-way interaction)
# Adjust terms based on above ANOVA results
eri_glmmTMB_simple <- update(eri_glmmTMB,
  . ~ year + elevation_rescaled + treatment + background_management + year:elevation_rescaled +
  elevation_rescaled:treatment + treatment:year + (1 | plotID) + (1 | site_name))
...
lup_glmmTMB_poly_simple <- update(lup_glmmTMB_poly,
  . ~ year + elevation_rescaled + treatment + background_management + year:poly(elevation_rescaled, 2)  + 
    poly(elevation_rescaled, 2):treatment + treatment:year + (1 | plotID) + (1 | site_name))
...
# All non sig., which means the simpler model fits the data better

# ANOVA tables for simplified models
...
# STEP 2 - drop second term
# Adjust terms based on above ANOVA results
eri_glmmTMB_simple2 <- update(eri_glmmTMB_simple,
  . ~ year + elevation_rescaled + treatment + year:elevation_rescaled +
  elevation_rescaled:treatment + treatment:year + (1 | plotID) + (1 | site_name))

sol_glmmTMB_simple2 <- update(sol_glmmTMB_simple,
  . ~ year + elevation_rescaled + treatment + year:elevation_rescaled +
  elevation_rescaled:treatment + treatment:year + (1 | plotID) + (1 | site_name))

lup_glmmTMB_poly_simple2 <- update(lup_glmmTMB_poly_simple,
  . ~ year + elevation_rescaled + treatment + background_management + year:poly(elevation_rescaled, 2)  + 
    treatment:year + (1 | plotID) + (1 | site_name))

# Compare full vs simplified models (Likelihood Ratio Tests)
cat("\n--- Simplified vs Simplified2 Model LRTs ---\n")
print(anova(eri_glmmTMB_simple, eri_glmmTMB_simple2))
print(anova(sol_glmmTMB_simple, sol_glmmTMB_simple2))
print(anova(lup_glmmTMB_poly_simple, lup_glmmTMB_poly_simple2))

# ANOVA tables for simplified models
cat("\n--- Simplified2 Model ANOVAs ---\n")
simpl_anovas <- list(
  erigeron = car::Anova(eri_glmmTMB_simple2),
  solidago = car::Anova(sol_glmmTMB_simple2),
  lupinus  = car::Anova(lup_glmmTMB_poly_simple2)
)
print(simpl_anovas)
# Let's keep Lupinus model

# STEP 3 - drop third term
# Adjust terms based on above ANOVA results
eri_glmmTMB_simple3 <- update(eri_glmmTMB_simple2,
  . ~ year + elevation_rescaled + treatment + year:elevation_rescaled +
  treatment:year + (1 | plotID) + (1 | site_name))

sol_glmmTMB_simple3 <- update(sol_glmmTMB_simple2,
  . ~ year + elevation_rescaled + treatment + year:elevation_rescaled +
  treatment:year + (1 | plotID) + (1 | site_name))

# Compare full vs simplified models (Likelihood Ratio Tests)
cat("\n--- Simplified2 vs Simplified3 Model LRTs ---\n")
print(anova(eri_glmmTMB_simple2, eri_glmmTMB_simple3))
print(anova(sol_glmmTMB_simple2, sol_glmmTMB_simple3))

# ANOVA tables for simplified models 
cat("\n--- Simplified3 Model ANOVAs ---\n")
simpl_anovas <- list(
  erigeron = car::Anova(eri_glmmTMB_simple3),
  solidago = car::Anova(sol_glmmTMB_simple3)
)
print(simpl_anovas)
# Let's keep also Erigeron model now, but one term of Solidago has still to be dropped

# STEP 4 - drop fourth term
# Adjust terms based on above ANOVA results
sol_glmmTMB_simple4 <- update(sol_glmmTMB_simple3,
  . ~ year + elevation_rescaled + treatment + 
  treatment:year + (1 | plotID) + (1 | site_name))

# Compare full vs simplified models (Likelihood Ratio Tests)
cat("\n--- Simplified3 vs Simplified4 Model LRTs ---\n")
print(anova(sol_glmmTMB_simple2, sol_glmmTMB_simple3))

# ANOVA tables for simplified models
cat("\n--- Simplified4 Model ANOVAs ---\n")
car::Anova(sol_glmmTMB_simple4)

# STEP 5 - drop fifth term
# Adjust terms based on above ANOVA results
sol_glmmTMB_simple5 <- update(sol_glmmTMB_simple4,
  . ~ year + treatment + year:treatment + (1 | plotID) + (1 | site_name))

# Compare full vs simplified models (Likelihood Ratio Tests)
anova(sol_glmmTMB_simple4, sol_glmmTMB_simple5)

# ANOVA tables for simplified models 
car::Anova(sol_glmmTMB_simple5)
# Most parsimonious model of Solidago reached as well

# Print Anova tables of final models
cat("\n--- Model ANOVAs of most parsimonious simplified models ---\n")
simpl_anovas <- list(
  erigeron = car::Anova(eri_glmmTMB_simple3),
  solidago = car::Anova(sol_glmmTMB_simple5),
  lupinus = car::Anova(lup_glmmTMB_poly_simple2)
)
print(simpl_anovas)
```
...
# Fig. 4: Slopegraphs with error bars 
...
cover_slopegraph_ci <- function(data, title, ylab, xlab){
  ggplot(
    ...
    labs(color = "Cover Group", linetype = "Treatment", shape = "Treatment") +
    my_theme +
...
# Export
ggsave("Figures/Fig_4_multi_funcGroup_slopegraph.png",
       plot = multi_slopegraph_ci, dpi = 700, units = "cm",
       height = 12.8, width = 21.5)
```

# Fig. S2 - Visualization of delta cover changes

```{r}
# Δ Cover violin + jitter plots
# Plotting function
...
# Plot Δ cover (treatment colors only: purple = Control, yellow = Removal)
p_delta <- ggplot(delta_df, aes(x = treatment, y = delta)) +
  ...
  geom_violin(aes(fill = treatment), width = 0.85, alpha = 0.35, color = NA, trim = TRUE) +

  # jittered points (colored by treatment)
...
# Save 
ggsave("Figures/Fig_S2_DeltaCover_ViolinJitter.png",
       p_delta, width = 21, height = 8, units = "cm", dpi = 600)
```

# Fig. S3 - species-specific correlation matrices between several variables at site sccale

```{r}
# Correlation Matrix: Environmental, Anthropogenic, and Survey Data at locations of study sites
# Load rasters from SWECO25 dataset once for entire script
...
for (sp in unique_species) {
  ...
  png(paste0("Figures/Fig_S3_CorrelationMatrix_", gsub(" ", "_", sp), ".png"),
...
png("Figures/Fig_S3_CorrelationMatrix_AllSites.png", width = 12, height = 11, units = "cm", res = 600)
