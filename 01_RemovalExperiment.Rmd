...
```{r}
# Erigeron annuus
lm_eri <- lm(cover_focal_sp_percentage ~ treatment,
             data = filter(eri_removal, year == 2021))
summary(lm_eri)

# Solidago canadensis
lm_sol <- lm(cover_focal_sp_percentage ~ treatment,
             data = filter(sol_removal, year == 2021))
summary(lm_sol)

# Lupinus polyphyllus
lm_lup <- lm(cover_focal_sp_percentage ~ treatment,
             data = filter(lup_removal, year == 2021))
summary(lm_lup)
```
...
```{r}
# Correlation Matrix with Raster Values
# Load required libraries
library(terra)      
library(corrplot)   

# Subset site coordinates and site name from full dataset
sites <- removal_data %>%
  dplyr::select(site_name, longitude, latitude, northing, easting, elevation) %>%
  dplyr::distinct(site_name, .keep_all = TRUE)

# Convert to spatial points (SpatVector)
sites_vect <- vect(sites, geom = c("longitude", "latitude"), crs = "EPSG:4326")

# Load raster layers and extract values
# Load rasters
r_precip <- readRDS("Data/ch_bioclim_chclim25_present_pixel_1981_2010_bio18_25.rds")
...
r_gdd    <- readRDS("Data/ch_bioclim_chclim25_present_pixel_1981_2010_gdd5.rds")
...
# Extract raster values for each site point
sites$precip      <- terra::extract(r_precip, sites_vect)[,2]
sites$temp        <- terra::extract(r_temp,   sites_vect)[,2]
sites$pop_density <- terra::extract(r_pop,    sites_vect)[,2]
sites$gdd         <- terra::extract(r_gdd,    sites_vect)[,2]

# Clean memory (large rasters)
rm(r_precip, r_temp, r_pop, r_gdd); gc()

# Prepare correlation matrix
env_df <- sites %>%
  dplyr::select(elevation, precip, temp, pop_density, gdd) %>%
  na.omit()

# Rename for readability
colnames(env_df) <- c("Elevation [m]", "Precipitation", "Temperature", "Pop. Density", "Growing Degree Days (GDD)")

# Compute Pearson correlation
cor_mat <- cor(env_df, method = "pearson")

# Plot correlation heatmap (upper triangle)
corrplot(cor_mat,
         method = "color",                          
         type = "upper",                            
         addCoef.col = "black",                     
