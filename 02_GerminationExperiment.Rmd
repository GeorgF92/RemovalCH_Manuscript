...
# Required R packages:
 ...
 - car: diagnostic tools and coefficient comparison
...
# Parameter interpretation
(resmodNegBin <- round(data.frame(summary(modNegBin_def)$coefficients,
                                  exp.Est = exp(modNegBin_def$coefficients),
                                  perc.Est = (exp(modNegBin_def$coefficients) - 1) * 100), digits = 3))
...
# Erigeron: Figures for Manuscript, with predicted lines from final NBRM-models
...
# Solidago: Figures for Manuscript, with predicted lines from final NBRM-models
...
plot_eri <- ggplot(eri_germ, aes(x = elevation, y = total_germinated)) +
  ...
  geom_line(data = pred_eri, aes(x = elevation, y = germ_pred), color = line_color_eri, size = 1) +  # CHANGED: uses conditional color
  labs(title = "Erigeron annuus",  # Title now italicized via theme
       x = "Elevation [m]",
       y = "Number of seedlings") +
  my_theme
...
plot_sol <- ggplot(sol_germ, aes(x = elevation, y = total_germinated)) +
  geom_point(alpha = 0.6) +
  geom_line(data = pred_sol, aes(x = elevation, y = germ_pred), color = line_color_sol, size = 1) +  # CHANGED: uses conditional color
  labs(title = "Solidago canadensis",  # Title now italicized via theme
       x = "Elevation [m]",
       y = "") + 
  my_theme
...
# Export
ggsave("Figures/Fig_Sx_TotalGermination_VS_Elevation.png", 
